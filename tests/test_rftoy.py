# RFToy Tests
"""
Simple tests for the RFToy library.
"""
import pytest
...
async def test_get_stations(httpx_mock):
    """
    Test to make sure we process the result of the /jc endpoint
    properly
    """
...
async def test_find_station(httpx_mock):
    """
    Test finding stations by name instead of ID
    """
    httpx_mock.add_response(url="http://sample.invalid/jc", json=DEFAULT_DATA)
    rftoy = RFToy(hostname="http://sample.invalid")
    await rftoy.repopulate()
...
async def test_on(httpx_mock):
    """
    Test to make sure the station status list
    is updated properly whe we call the `on` method.
    """
...
async def test_off(httpx_mock):
    """
    Test to make sure the station status list
    is updated properly whe we call the `off` method.
    """
